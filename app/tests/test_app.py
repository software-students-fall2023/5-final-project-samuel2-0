...
def test_login_route(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda query: {"_id": ObjectId(user_id), "email": "test@example.com", "password": "password"},
    )
...
def test_update_interests(client, monkeypatch):
    ...
    response = client.post('/profile/edit_interests', data={"interests[]": new_interests})
...
def test_update_languages(client, monkeypatch):
    ...
    response = client.post('/profile/edit_languages', data={'languages[]': ['Spanish', 'German']})
...
def test_my_friends(client, monkeypatch):
    ...
    monkeypatch.setattr("app.get_user", mock_get_user)
    user_id = ObjectId()
    mock_get_user.return_value = {"_id": user_id, "username": "testuser", "friends": ["friend1", "friend2"]}
...
def test_pal_profile(client, monkeypatch):
    ...
    mock_get_user.side_effect = lambda user_id: current_user if str(user_id) == str(current_user["_id"]) else friend_user
    monkeypatch.setattr("app.render_template", Mock(return_value="rendered_template"))
    response = client.get('/pal_profile', query_string={"user_id": str(friend_user["_id"])})
    assert response.status_code == 200
...
def test_see_friend_missing_friend_id(client, monkeypatch):
    with client.session_transaction() as session:
        session["userid"] = str(ObjectId())
    monkeypatch.setattr("app.get_user", lambda user_id: {"_id": ObjectId(user_id), "username": "testuser"})
